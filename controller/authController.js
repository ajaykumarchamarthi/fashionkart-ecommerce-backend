...
exports.signup = catchAsync(async (req, res) => {
  ...
  const url = 0; // Profile Image link `${req.protocol}://${req.get('host')}/me`
...
exports.protect = catchAsync(async (req, res, next) => {
  // 1) Getting token & check if there is token
  let token;
  if (
  ...
  if (!freshUser) {
    next(new AppError(`The token belonging to this user doesn't exist`, 401));
  }

  // 4) Check if user changed password after the token is issued
  if (freshUser.changedPasswordAfter(decoded.iat)) {
    return next(
      new AppError(`User recently changed password! Login in again`, 401)
    );
  }
